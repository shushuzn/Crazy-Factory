...
const createMarketSystem = ({
  ...
  const renderMarket = () => {
    ...
    if (marketOutlookEl) {
      ...
      marketOutlookEl.textContent = `利率前瞻：${st.rateOutlookDirection}${arrow}（置信 ${st.rateOutlookConfidence || 0}%｜命中 ${hitRatePct}% ${hits}/${hits + misses}｜预案 ${st.lastAutoPlanTarget || 'none'}）`;
    }
  };
