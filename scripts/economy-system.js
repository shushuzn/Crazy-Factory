...
const createEconomySystem = ({
  ...
  const autoPlanTarget = () => {
    const preferred = st.macroPreferredBuildingId ? bld(st.macroPreferredBuildingId) : null;
    if (preferred && isBldUnlocked(preferred)) return preferred;
    return [...buildings].reverse().find((x) => isBldUnlocked(x)) || buildings[0];
  };
  ...
  const tryAutoBuy = () => {
    ...
    const target = autoPlanTarget();
    if (target && st.lastAutoPlanTarget !== target.id) {
      pushLog(`🤖 自动预案切换：${target.name}${st.macroPreferredBuildingId ? '（宏观偏好）' : '（常规）'}`);
      st.lastAutoPlanTarget = target.id;
      dirty.logs = true;
    }

    const candidates = target
      ? [target, ...[...buildings].reverse().filter((x) => x.id !== target.id)]
      : [...buildings].reverse();
