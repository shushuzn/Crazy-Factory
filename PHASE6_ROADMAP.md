...
**优先级**: 🔴 高  
**预计工期**: 3-4 周  
**前置依赖**: P6-T2 产业链联动（已前置完成）  
**进度**: Week 1 基础架构已完成，核心逻辑已实现

### ✅ 已完成任务

- [x] **1. 创建 derivatives-system.js 模块** (scripts/derivatives-system.js)
  - 模块工厂函数 `createDerivativesSystem()`
  - 数据持久化结构 (st.derivatives)
  - 事件总线集成 (eventBus)
  - 多语言支持 (I18N)

- [x] **2. 定义期货/期权数据结构**
  - 期货合约结构：id, type, leverage, margin, entryPrice, status
  - 期权合约结构：id, type, strikePrice, premium, expiresAt
  - 对冲配置：autoHedge, hedgeRatio
  - 交易统计：totalTrades, profit/loss tracking

- [x] **3. Week 2 核心逻辑** (已实现)
  - 保证金计算：`margin = contractValue * 0.1`
  - 杠杆机制：支持 2x, 5x, 10x 杠杆
  - 爆仓/强平逻辑：维持保证金 5%，触发强平
  - 未实现盈亏实时计算
  - 与市场系统联动 (market.getCurrentIndex)
...
- [x] **5. UI 面板与集成**
  - 期货持仓面板 `renderFuturesPanel()`
  - 期权持仓面板 `renderOptionsPanel()`
  - 交易统计面板 `renderStatsPanel()`
  - game.js 集成与事件监听
  - 调试命令：openFutures, closeFutures, buyOptions, getDerivativesStats
...
**预计工期**: 1-2 周  
**前置依赖**: 无（可独立实施）  
**进度**: 核心框架已实现，5种危机类型已配置，GPS集成已完成

### ✅ 已完成任务

- [x] **1. 创建 crisis-system.js 模块**
  - 模块工厂函数 `createCrisisSystem()`
  - 数据持久化结构 (st.crisis)
  - 事件总线集成
  - 多语言支持 (I18N)

- [x] **2. 定义危机类型和效果**
  - **金融危机**: GPS减半，手动收益减半，建筑成本+30%，持续5分钟
  - **全球疫情**: 手动收益30%，离线收益20%，建筑效率-20%，持续10分钟
  - **网络攻击**: 禁用自动购买，市场冻结，技能冷却翻倍，持续2分钟
  - **贸易战**: 产业链加成失效，套利禁用，全球市场加成减半，持续4分钟
  - **恶性通胀**: 每秒流失1%资金，价格翻倍，负利率，持续3分钟

- [x] **3. 危机触发逻辑**
  - 基于概率的随机触发（每日0.03%-0.1%）
  - 资金门槛机制（不同危机不同要求）
  - 免疫期机制（30分钟，防止连续危机）
  - 0.1秒间隔检查
...
- [x] **5. UI面板与集成**
  - 危机状态面板 `renderCrisisPanel()`（显示在顶部）
  - 历史记录面板 `renderCrisisHistory()`
  - 实时倒计时条
  - 救助按钮（资金充足时可用）
  - economy-system.js GPS集成 `setCrisisMultiplier()`
  - game.js 集成与事件监听
  - 调试命令: triggerCrisis, recoverCrisis, getCrisisInfo
...
**优先级**: 🟢 低  
**预计工期**: 3-4 周  
**前置依赖**: 其他所有 Phase 6 任务完成（已全部完成）  
**进度**: 核心框架已实现，5个虚拟公会已配置，GPS集成已完成

### ✅ 已完成任务

- [x] **1. 创建 guild-system.js 模块**
  - 模块工厂函数 `createGuildSystem()`
  - 数据持久化结构 (st.guild, st.virtualGuilds)
  - 事件总线集成
  - 多语言支持 (I18N)

- [x] **2. 定义虚拟公会数据**
  - **阿尔法交易团** (交易型): GPS+15%, 套利+20%
  - **贝塔投资联盟** (投资型): 投资+25%, 离线+15%
  - **伽马持有会** (平衡型): 全加成+10%
  - **德尔塔科技社** (科技型): 研究+20%, 建筑成本-10%
  - **艾普西隆全球** (平衡型): 全球市场+15%, 全加成+5%

- [x] **3. 公会加入/退出机制**
  - `joinGuild(guildId)` 加入公会
  - `leaveGuild()` 退出公会
  - 成员数实时更新
  - 24小时冷却期（退出后）

- [x] **4. 公会贡献系统**
  - 主动贡献：`contribute(amount)`
  - 自动贡献：每秒0.01% GPS
  - 贡献历史记录
  - 公会升级机制（10级）
...
- [x] **6. 公会加成效果**
  - GPS加成、投资加成、研究加成等
  - 升级时加成增强10%
  - economy-system.js GPS集成 `setGuildMultiplier()`
...
- [x] **7. UI面板与集成**
  ...
  - 公会排名面板
  - game.js 集成与事件监听
  - 调试命令: joinGuild, leaveGuild, contributeToGuild, getGuildInfo
