...
| 版本 | 主题 | 核心交付 | 验收门槛 |
| --- | --- | --- | --- |
| v0.9 | 金融系统深化 | 完成 `M8-T05`（市场事件 + 利率前瞻）；新增 2 类宏观事件与事件预告条 | 10 分钟试玩中事件链至少触发 3 次，且事件日志可追溯 |
| v1.0 | 策略层成型 | 新增“资产配置/风险偏好”系统（可影响自动购买与收益波动）；补齐教程引导 | 新手 15 分钟留存流程可完整跑通（无阻塞），关键系统均有可视化提示 |
| v1.1 | 长线内容扩展 | 引入“季度目标 + 赛季结算”循环；新增中后期可重复挑战内容 | 180 天模拟下 CAGR、回撤、破产指标不劣化；自动化脚本可复现 |
| v1.2 | 发布候选（RC） | 性能与可维护性收口：存档兼容策略、回归脚本集成、版本日志自动生成 | 通过全量测试 + 30 分钟 soak；版本发布清单（测试/回滚/已知问题）完整 |

### 版本节奏与发布规则
- 双周节奏：每 2 周冻结 1 个候选版本，按 MVI 原则只上线 1 个主任务。
- 门禁规则：必须通过 `tests` + `run_soak_check` + 路线图验收项，方可标记 `DONE`。
- 风险规则：若 `Risk Level=高` 或 North Star 连续两轮下降，版本状态切换为 `Recovery`，暂停新功能。
